...
def recommendation(
    API_KEY_ALPHAVANTAGE: str, ticker: str, screener: str, exchange: str, interval: str
) -> str:
    ...
    if not exchange:
        s_req = f"https://www.alphavantage.co/query?function=OVERVIEW&symbol={ticker}&apikey={API_KEY_ALPHAVANTAGE}"
    ...
    if not interval:
        ...
        d_recommendation = stock_recommendation.get_analysis().summary
        recom = f"Interval: {INTERVALS[interval]}\n"
        recom += f"Recommendation: {d_recommendation['RECOMMENDATION']}\n"
        recom += f"{int(d_recommendation['BUY'])} BUY, {int(d_recommendation['NEUTRAL'])} NEUTRAL"
        recom += f", {int(d_recommendation['SELL'])} SELL"
